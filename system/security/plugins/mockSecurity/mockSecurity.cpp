...
{
    using Resources = std::map<SecResourceType, Owned<ISecResourceList>>;
    Owned<ISecUser>         user;
    Owned<ISecResourceList> settings;
    Resources               permissions;
};

struct UserCache : public CInterface
{
    ...
    using UserCacheMap = std::map<std::string, UserCacheEntry, KeyComparator>;
    UserCacheMap cache;

    /**
     * @brief Return a reference to a new or preexisting cache entry keyed by name.
     *
     * An existing entry will be returned if found. A new entry will be created otherwise.
     */
    UserCacheEntry* ensureEntry(const char* name);

    /**
     * @Brief Return a reference to a preexisting cache entry keyed by name.
     */
    UserCacheEntry* getEntry(const char* name);
};

class CMockSecurityManager : public CBaseSecurityManager
{
public:
    CMockSecurityManager(const char * serviceName, IPropertyTree * secMgrCfg, IPropertyTree * bindConfig) : CBaseSecurityManager(serviceName, (IPropertyTree *)NULL)
    {
        if (secMgrCfg)
        {
...
extern "C"
{
    MOCKSECURITY_API ISecManager * createInstance(const char * serviceName, IPropertyTree &secMgrCfg, IPropertyTree &bndCfg)
    {
        return new CMockSecurityManager(serviceName, &secMgrCfg, &bndCfg);
    }
