...
{
    OwnedSpanScope resultSpanScope(queryThreadedActiveSpan()->createClientSpan("esp.ws_sql.wu_result"));
    try
    {
        if (wuid && *wuid)
        {
            ...
            case WUStateCompiled:
            {
                ...
                if (nr.get())
                {
                    OwnedSpanScope xmlSpanScope(queryThreadedActiveSpan()->createClientSpan("xml_results"));
                    try
                    {
...
{
    OwnedSpanScope exSpanScope(queryThreadedActiveSpan()->createClientSpan("esp.ws_sql.execute_sql"));
    try
    {
        ...
        {
            {
                ...
                {
                    OwnedSpanScope eclGenSpanScope(queryThreadedActiveSpan()->createClientSpan("ecl_generate");
                    try
                    {
                        ::generateECL(parsedSQL, ecltext);
                ...
                OwnedSpanScope wuCompileSpanScope(queryThreadedActiveSpan()->createClientSpan("wu_compile"));
                try
                {
        ...
        else
        {
            ...
            if (clonable)
            {
                OwnedSpanScope wuCloneSpanScope(queryThreadedActiveSpan()->createClientSpan("wu_clone"));
                try
                {
            ...
            {
                OwnedSpanScope wuSubmitSpanScope(queryThreadedActiveSpan()->createClientSpan("wu_submit"));
                try
                {
            ...
            if (timeToWait != 0)
            {
                OwnedSpanScope wuProcessSpanScope(queryThreadedActiveSpan()->createClientSpan("wu_process"));
                try
                {
